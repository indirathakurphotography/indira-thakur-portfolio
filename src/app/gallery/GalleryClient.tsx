...
import { normalizeCategory, isCategoryMatch, formatCategory } from '@/lib/categoryUtils';
    ...
    >
      <div className="relative overflow-hidden bg-[#FAF6F3] rounded-sm">
        <img
          ...
          className="w-full h-auto block object-cover transition-all duration-500 group-hover:scale-[1.03]"
        />
...
export default function GalleryClient() {
  ...
  // Sync activeCategory when searchParams changes (e.g. navigation via URL query)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchGallery = async () => {
      try {
        const res = await fetch('/api/gallery-images?page=1&limit=500');
        if (!res.ok) return;
  ...
  const availableCategories = useMemo(() => {
    ...
    galleryImages.forEach((img) => {
      if (img.category) {
        ...
        if (norm && !catsMap.has(norm)) {
          catsMap.set(norm, img.category);
        }
      }
    });
  ...
  const filtered = useMemo(() => {
    ...
    return (!activeCategory || normalizedActiveCategory === '' || normalizedActiveCategory === 'all')
      ? galleryImages
      : galleryImages.filter((img) => isCategoryMatch(img.category, activeCategory));
            ...
            <div className="space-y-16 md:space-y-24">
              {groupedShoots.map((shootGroup, groupIdx) => (
                <div key={shootGroup.title || groupIdx} className="space-y-6 md:space-y-8">
                  ...
                  {/* Responsive Masonry Grid for Shoot */}
