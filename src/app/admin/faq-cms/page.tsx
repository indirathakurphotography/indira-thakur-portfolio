...
export default function AdminFAQPage() {
  ...
  const faqsToRender = Array.isArray(faq.faqs) ? faq.faqs : (Array.isArray(faq.items) ? faq.items : defaultFaqs);

  const handleFaqChange = (index: number, field: string, value: string) => {
    const items = [...faqsToRender];
    items[index] = { ...items[index], [field]: value };
    updateSection('faq', { faqs: items, items: items });
  };

  const addFaq = () => {
    const items = [...faqsToRender, { question: '', answer: '' }];
    updateSection('faq', { faqs: items, items: items });
  };

  const removeFaq = (index: number) => {
    const items = faqsToRender.filter((_: any, i: number) => i !== index);
    updateSection('faq', { faqs: items, items: items });
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto w-full pb-20">
      ...
      <div className="space-y-6">
        ...
        <Section title={`FAQs (${faqsToRender.length})`} defaultOpen icon={<HiQuestionMarkCircle className="w-5 h-5" />}>
          ...
          <div className="space-y-4">
            {faqsToRender.map((item: any, i: number) => (
