...
import { MAX_IMAGE_UPLOAD_SIZE, MAX_VIDEO_UPLOAD_SIZE, MAX_IMAGE_UPLOAD_SIZE_MB, MAX_VIDEO_UPLOAD_SIZE_MB } from '@/lib/uploadConstants';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const maxBytes = file.type.startsWith('video/') ? MAX_VIDEO_UPLOAD_SIZE : MAX_IMAGE_UPLOAD_SIZE;
      if (file.size <= 0 || file.size > maxBytes) {
        return jsonError(`File is empty or exceeds the ${file.type.startsWith('video/') ? `${MAX_VIDEO_UPLOAD_SIZE_MB} MB video` : `${MAX_IMAGE_UPLOAD_SIZE_MB} MB image`} limit.`, 413);
      }
