  ...
  try {
    if (process.env.MONGODB_URI) {
      try {
        ...
        const items = await (VideoTestimonial as any).find({})
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (process.env.MONGODB_URI) {
      await connectToDatabase();
      const updated = await (VideoTestimonial as any).findByIdAndUpdate(
        targetId,
        { $set: body },
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (process.env.MONGODB_URI) {
      await connectToDatabase();
      const deleted = await (VideoTestimonial as any).findByIdAndDelete(id);
