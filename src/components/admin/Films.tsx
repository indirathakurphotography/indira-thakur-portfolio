...
import { HiPlus, HiTrash, HiPencil, HiVideoCamera, HiXMark, HiPhoto, HiExclamationCircle, HiCheckCircle } from 'react-icons/hi2';
...
export function FilmsCMS() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const autoThumb = formData.thumbnailUrl.trim() || getVideoThumbnail(formData.videoUrl.trim());
    ...
    const validation = validateFilmPayload(payload);
    if (!validation.valid) {
      setError(validation.error || 'Invalid film data');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(resData.error || 'Failed to save film');
      }
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Film video upload failed');
    } finally {
  ...
  const getVideoSourceLabel = (url: string) => {
    if (!url) return null;
    if (url.includes('/videos/testimonials/') || url.includes('/testimonials/')) {
      return { label: 'Invalid: Belongs to Video Testimonials', valid: false };
    }
    if (url.includes('/home/hero/') || url.includes('/slideshow/')) {
      return { label: 'Invalid: Belongs to Home Hero Slideshow', valid: false };
    }
    if (/youtu(\.be|be\.com)/i.test(url)) return { label: 'YouTube Embed Link', valid: true };
    if (/drive\.google\.com/i.test(url)) return { label: 'Google Drive Video Stream', valid: true };
    if (/vimeo\.com/i.test(url)) return { label: 'Vimeo Video Stream', valid: true };
    if (/\.(mp4|webm|mov|m4v)(\?.*)?$/i.test(url) || url.includes('/storage/v1/object/public/images/films/')) {
      return { label: 'Direct Film Video (MP4/WebM)', valid: true };
    }
    return { label: 'Custom Video Stream', valid: true };
  };

  const videoSourceInfo = getVideoSourceLabel(formData.videoUrl);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {error && (
        <div className="p-4 mb-6 bg-rose-50 border border-rose-200 text-rose-700 text-sm rounded flex items-start gap-3">
          <HiExclamationCircle className="w-5 h-5 text-rose-500 shrink-0 mt-0.5" />
          <div>
            <p className="font-medium">Film Validation Error</p>
      ...
      {showForm && (
        <div className="bg-white border border-cream rounded-lg p-6 mb-8 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs uppercase tracking-wider text-warm-gray mb-2">Film Video Source (Google Drive / YouTube / Vimeo / MP4 Upload) *</label>
                <div className="flex gap-2 items-center mb-2">
                  <input
                    ...
                    required
                  />
                  <label className="px-4 py-3 bg-cream hover:bg-cream/80 text-rich-black text-xs uppercase tracking-wider rounded cursor-pointer flex items-center gap-2 shrink-0">
                    <HiVideoCamera className="w-4 h-4" /> Upload MP4
                    <input type="file" accept="video/mp4,video/webm,video/quicktime" onChange={handleVideoUpload} className="hidden" />
                  </label>
                </div>
                {uploadingVideo && <p className="text-xs text-magenta mb-2">Uploading film video to /images/films/...</p>}
                {videoSourceInfo && (
                  <div className={`mt-1 text-xs px-3 py-1.5 rounded flex items-center gap-2 ${
                    videoSourceInfo.valid
                      ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                      : 'bg-rose-50 text-rose-700 border border-rose-200'
                  }`}>
                    {videoSourceInfo.valid ? <HiCheckCircle className="w-4 h-4" /> : <HiExclamationCircle className="w-4 h-4" />}
                    <span>{videoSourceInfo.label}</span>
                  </div>
                )}
              </div>
            ...
            <div>
              <label className="block text-xs uppercase tracking-wider text-warm-gray mb-2">Thumbnail Poster Image</label>
              <div className="flex gap-4 items-center">
                <input
                  ...
                  placeholder="https://.../storage/v1/object/public/images/films/..."
                  className="flex-1 px-4 py-3 bg-white border border-cream rounded text-sm focus:outline-none focus:border-magenta"
                />
                <label className="px-4 py-3 bg-cream hover:bg-cream/80 text-rich-black text-xs uppercase tracking-wider rounded cursor-pointer flex items-center gap-2 shrink-0">
                  <HiPhoto className="w-4 h-4" /> Upload Image
                  <input type="file" accept="image/*" onChange={handleThumbnailUpload} className="hidden" />
                </label>
              </div>
              {uploading && <p className="text-xs text-magenta mt-1">Uploading thumbnail image to /images/films/...</p>}

              {formData.thumbnailUrl && (
                <div className="mt-3 relative w-40 aspect-video bg-charcoal rounded overflow-hidden border border-cream shadow-sm">
                  ...
                  <span className="absolute bottom-1 left-1 bg-black/70 text-white text-[9px] px-1.5 py-0.5 rounded font-mono">Preview</span>
                </div>
              )}
            </div>
