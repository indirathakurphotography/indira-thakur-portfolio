...
function getInstagramEmbedUrl(url: string): string {
  try {
    ...
    const mediaType = parts[mediaTypeIndex]?.toLowerCase() === 'reels' ? 'reel' : parts[mediaTypeIndex]?.toLowerCase();
...
export default function InstagramReels({ category, home = false }: { category: string; home?: boolean }) {
  ...
  const card = (item: Item, key: string) => (
    <div key={key} className="w-[280px] shrink-0 bg-[#151211] aspect-[9/14] overflow-hidden relative">
      {item.mediaType === 'video' ? (
        <video src={item.url} poster={item.thumbnailUrl} controls className="w-full h-full object-cover" />
      ) : (
        <iframe
  ...
  return (
    <section className={home ? 'py-16 overflow-hidden bg-[#151211]' : 'border-t border-[#E7DDD2] pt-14'}>
      ...
      {home ? (
        <div className="flex gap-5 w-max animate-[instagramScroll_45s_linear_infinite]">
